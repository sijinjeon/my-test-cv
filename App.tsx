...
const Hero: React.FC = () => (
  <section className="pt-20 pb-12 px-6 max-w-7xl mx-auto text-center animate-fade-in">
      ...
      <div className="md:col-span-6 flex justify-center relative">
        ...
        <div className="absolute top-1/2 -left-12 -translate-y-1/2 opacity-20 pointer-events-none hidden md:block">
           <svg width="60" height="100" viewBox="0 0 60 100">
              <path d="M0 10 Q 15 0, 30 10 T 60 10" fill="none" stroke="black" strokeWidth="1" />
              <path d="M0 30 Q 15 20, 30 30 T 60 30" fill="none" stroke="black" strokeWidth="1" />
              <path d="M0 50 Q 15 40, 30 50 T 60 50" fill="none" stroke="black" strokeWidth="1" />
           </svg>
        </div>
      </div>
  ...
  <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
    {CV_DATA.activities.map((activity, i) => (
      <div key={i} className="space-y-4 group">
        <span className="text-[10px] font-bold tracking-widest uppercase text-gray-400 px-2 py-1 border border-gray-200 rounded group-hover:bg-black group-hover:text-white group-hover:border-black transition-colors inline-block">
          {activity.tag}
        </span>
...
const ProjectModal: React.FC<{ project: Project; onClose: () => void }> = ({ project, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-8 bg-black/80 backdrop-blur-sm" onClick={onClose}>
      <div 
        className="bg-white w-full max-w-4xl max-h-[90vh] overflow-y-auto relative animate-fade-in"
